...
const updateStock_ProductService = async (items) => {
  await axios.post(
    "http://localhost:5000/api/products/productList/updateStock",
    { items }
  );
};
...
const getUserSpecific = async (req, res) => {
  try {
    ...
    const productResponse = await axios.post(
      "http://localhost:5000/api/products/productlist/getBulk_userSpecific",
      {
        ids: productIds,
      }
    );
...
const getStatusCounts = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Error fetching order status counts", error });
  }
};
